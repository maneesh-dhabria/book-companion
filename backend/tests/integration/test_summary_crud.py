...
async def book_with_sections(db_session):
    ...
    book = Book(
        title="Test Book", file_data=b"test", file_hash="test_hash_summary_crud",
...
async def test_list_book_level_summaries(db_session, book_with_sections):
    ...
    await repo.create(Summary(
        content_type=SummaryContentType.BOOK, content_id=book.id,
        book_id=book.id, facets_used={}, prompt_text_sent="t", model_used="s",
        input_char_count=100, summary_char_count=20, summary_md="Book S",
    ))
    await repo.create(Summary(
        content_type=SummaryContentType.SECTION, content_id=sections[0].id,
        book_id=book.id, facets_used={}, prompt_text_sent="t", model_used="s",
        input_char_count=100, summary_char_count=20, summary_md="Section S",
    ))
