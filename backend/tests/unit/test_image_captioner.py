...
def test_skip_decorative_filename():
    assert (
        should_skip_image(data=b"x" * 10000, width=200, height=200, filename="separator.png")
        is True
    )
    assert (
        should_skip_image(data=b"x" * 10000, width=200, height=200, filename="header-divider.gif")
        is True
    )
    assert (
        should_skip_image(data=b"x" * 10000, width=200, height=200, filename="bullet_icon.png")
        is True
    )
...
def test_skip_cover_image():
    ...
    assert (
        should_skip_image(data=b"x" * 50000, width=600, height=900, filename="book-cover.png")
        is True
    )
...
def test_keep_normal_image():
    assert (
        should_skip_image(data=b"x" * 10000, width=400, height=300, filename="figure_1.png")
        is False
    )
...
def test_keep_image_with_no_dimensions():
    assert (
        should_skip_image(data=b"x" * 10000, width=None, height=None, filename="chart.png") is False
    )
...
async def test_dedup_skips_already_captioned_hash():
    ...
    images = [
        {
            ...
            "alt_text": None,
            "content_hash": "abc123",
            "existing_caption": "A framework diagram.",
    ...
    results = await captioner.caption_section_images(images=images, section_context="Test")
