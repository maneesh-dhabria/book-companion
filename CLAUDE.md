...
- **LLM via subprocess**: `ClaudeCodeCLIProvider` invokes `claude -p - --output-format json --print` with prompt piped via stdin and structured output via `--json-schema`. `CodexCLIProvider` invokes `codex exec --skip-git-repo-check --output-schema <FILE> --output-last-message <FILE>` with prompt piped via stdin (`-`); the schema is written to a temp file and the agent's final message is read back from `--output-last-message`. Both providers receive a `config_dir: Path | None` and inject `CLAUDE_CONFIG_DIR` / `CODEX_HOME` into the subprocess env.
...
12. **Codex CLI uses native schema flags (v1.6+)**: `CodexCLIProvider.generate()` writes the JSON schema to a temp file and passes `--output-schema <FILE> --output-last-message <FILE>` to `codex exec`. The agent's structured response is read from the `--output-last-message` file (NOT stdout), so don't try to parse the subprocess stdout for schema-conforming JSON. Both temp files are cleaned up in a `try/finally`. Codex 0.118+ does NOT accept `--output-format json` — that flag does not exist.
...
21. **TTS engine: optional `[kokoro]` extra**: `kokoro-onnx` and `soundfile` ship as `pip install bookcompanion[kokoro]` (declared in `pyproject.toml`'s `[project.optional-dependencies]`). Default install only supports the Web Speech engine. The Kokoro path also requires `ffmpeg` and `espeak-ng` on PATH — `bookcompanion init --tts-engine kokoro` gates on both with actionable hints. Model files (`kokoro-v1.0.onnx`, `voices-v1.0.bin`, ~338 MB) are downloaded from `github.com/thewh1teagle/kokoro-onnx/releases/download/model-files-v1.0/...` to `data_dir/models/tts/`. Streaming download writes a `.partial` sibling and atomic-renames; a half-finished file can't poison subsequent loads. The HF repo `onnx-community/Kokoro-82M-v1.0-ONNX` ships shards in a layout `kokoro-onnx` doesn't load — don't pivot back to it.
22. **TTS env vars (Kokoro)**: at runtime, Kokoro's phonemizer needs `PHONEMIZER_ESPEAK_LIBRARY` and `ESPEAK_DATA_PATH` to point at the espeak-ng install. On macOS via Homebrew: `/opt/homebrew/Cellar/espeak-ng/<ver>/lib/libespeak-ng.dylib` and `/opt/homebrew/Cellar/espeak-ng/<ver>/share/espeak-ng-data`. Without them, the first synth raises a cryptic "phontab not found" deep in onnxruntime.
23. **TTS SSE event names**: the audio worker emits `section_audio_completed`, `section_audio_failed`, `section_audio_already_stale` (NOT the summarize-step `section_completed` etc.). `useBufferedJobStream.applyEvent` has dedicated cases for these — adding a new audio-step event requires updating both the backend emit site and the frontend switch + `SseEvent.event` union.
24. **TTS content type unions diverge by purpose**: `AudioContentType` (`@/api/audio`) is the persistable subset (`section_summary`/`book_summary`/`section_content`/`annotations_playlist`) — matches the backend `ContentType` enum and the `audio_files`/`audio_positions` CHECK constraints. `TtsContentType` (`@/stores/ttsPlayer`) is a superset that adds `'annotation'` for runtime engine routing only (single-annotation Web Speech playback, never persisted). Sending `'annotation'` to `/audio_position` or `/audio/lookup` will 400.
25. **TtsEngineKind values are `mp3` and `web-speech` — NOT `kokoro`**: The Kokoro engine identifies as `'mp3'` at runtime because it plays precomputed MP3 files generated by Kokoro. Use `engine !== 'web-speech'` for Kokoro-or-else gates instead of `=== 'kokoro'`. The `EngineChip` component knows about both spellings (`isKokoro` matches `'kokoro' || 'mp3'`), but the store-level `TtsEngineKind` does not.
26. **SettingsView route is `/settings/:section?`**: `/settings` alone (no section param) falls through to `GeneralSettings`, NOT the audio panel. Use `/settings/tts#audio` for audio-panel deep-links — `SettingsTtsPanel.vue`'s root carries `id="audio"`, so the hash anchor scrolls to the engine settings.
27. **`/reading-state/continue` is cross-device only; `/reading-state/resume-banner` is not**: The `/continue` endpoint filters by user-agent (excludes the writing device), so on the device that wrote the position it returns null. The `/resume-banner` endpoint returns the latest reading + audio activity across ALL devices. `ContinueBanner` self-gates on `/continue`, so on the writing device it renders empty even when the resume-banner store reports activity. Don't be surprised by an empty banner mounted via `v-if="resumeBanner.chosen === 'reading'"` on the same device.
28. **Pinia setup-store with Map state needs `reactive(new Map())`, NOT `ref<Map>(new Map())`**: Map collection ops (`.get`/`.set`/`.delete`) on a `ref` value don't proxy through Vue's reactive system the same way `reactive(Map)` does — deep mutations to inner state objects (e.g., `state.sessions.push(x)` or `state.currentQuestion = q`) silently fail to invalidate dependent computeds. Pattern: `const byBook = reactive(new Map())` and per-key state wrapped with `state = reactive(emptyState())` so inner property writes track. Caught silently in T26 `quizSessions` store; debug script showed correct state but `pastSessions` computed returned `[]`.
29. **CLI tests must reset `app.cli.deps._settings` module cache when monkeypatching env vars**: `app/cli/deps.py:_settings` is module-level cached. After `monkeypatch.setenv("BOOKCOMPANION_DATA__DIRECTORY", ...)`, also call `monkeypatch.setattr(cli_deps, "_settings", None)` — otherwise the CLI's `get_services()` sees the prior test's `Settings` and connects to the wrong DB. Surfaces as "no such table" errors in full-suite runs while the test passes in isolation. T34's `test_cli_export_quiz_session_writes_file` is the canonical case.
30. **vitest with explicit props masks mount-site prop drift**: When adding optional props with defaults to a frequently-mounted component (e.g., `GenerateAudioModal`), audit each call site or add a smoke test that exercises the live mount — unit tests that pass props directly will green-light a UX-distorting integration gap. Caught in `/verify` for `quiz-audio-ux-top7` (Gap 5): `AudioTab` passed only 4 of 7 props to `GenerateAudioModal`, so the defaults made the estimate row render a 0-min / 0-MB build. The same shape bit the `ApiError` → `InlineDiagnostic` path — tests set `stderrTail` directly, but the live response-body unwrap didn't propagate `llm_stderr_tail` to the prop.
