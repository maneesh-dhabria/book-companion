 ...
 */
export function useReadingState(bookId: () => number | undefined, sectionId: () => number | undefined) {
  const store = useReadingStateStore()

  onMounted(() => {
    const bid = bookId()
    if (bid) {
      store.trackPosition(bid, sectionId())
    }
  })

  watch(
    [bookId, sectionId],
    ([newBookId, newSectionId]) => {
      if (newBookId) {
        store.trackPosition(newBookId, newSectionId)
      }
    },
  )
}
