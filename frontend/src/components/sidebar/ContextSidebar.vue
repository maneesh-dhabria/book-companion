...
<style scoped>
.context-sidebar { width: 360px; min-width: 360px; border-left: 1px solid var(--color-border, #e0e0e0); background: var(--color-bg, #fff); display: flex; flex-direction: column; height: 100%; overflow: hidden; }
.sidebar-header { display: flex; justify-content: space-between; align-items: center; padding: 0.5rem 0.75rem; border-bottom: 1px solid var(--color-border, #e0e0e0); }
.tab-bar { display: flex; gap: 0; }
.tab { padding: 0.5rem 0.75rem; border: none; background: none; cursor: pointer; font-size: 0.8rem; font-weight: 500; color: var(--color-text-secondary, #666); border-bottom: 2px solid transparent; }
