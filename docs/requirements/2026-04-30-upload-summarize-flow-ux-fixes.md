...
4. **Free-form `cli_command` input is a foot-gun and silently breaks summarization.** The Settings → LLM page exposes a free-form text input for `cli_command`. The user typed `claude-personal` (a zsh alias defined in `~/.zshrc`), which works in an interactive shell but does NOT work for non-interactive `subprocess.run(['claude-personal', ...])` — `subprocess` does not source rc files, so aliases and shell functions are invisible. Every section's call failed with "binary not found." The summarize job returned HTTP 200 and only logged per-section failures; the user got no clear signal that the configured CLI was the root cause.
   ...
   **Implementation note (verified empirically against the maintainer's installed CLIs on 2026-04-30):**
   - **Both providers are env-var driven for profile isolation** (no CLI flags involved):
     - `claude` (2.1.123): `CLAUDE_CONFIG_DIR=<path>` — `claude --help` exposes no `--config-dir` flag.
     - `codex` (0.118.0): `CODEX_HOME=<path>`.
   - Live test: `subprocess.run(['claude', '-p', '-', '--output-format', 'json', ...], env={..., 'CLAUDE_CONFIG_DIR': '~/.claude-personal'})` returned `{"result": "OK"}` and the session was written into `~/.claude-personal/projects/...` (default `~/.claude/projects/` unchanged). Same outcome for codex with `CODEX_HOME` — `~/.codex-personal/state_5.sqlite` updated at test time, default `~/.codex/` untouched.
   - **`codex exec` requires `--skip-git-repo-check`** when invoked outside a git repo. Book Companion's worker runs from the data dir, not a repo; `CodexCLIProvider` must pass this flag (or its config.toml equivalent).
   - The `LLMConfig` pydantic model currently has no `config_dir` field (and uses `extra="forbid"`), so existing YAML keys like `config_dir: ~/.claude-personal` are rejected at load — the user's intent has been silently ignored. New field required: `LLMConfig.config_dir: Path | None`.
   - Implementation is symmetric: both providers inject the path into `subprocess.create_subprocess_exec(env=...)` under the appropriate variable name. No CLI-flag wiring needed for either.
...
| D9 | Remove the free-form `cli_command` text input from Settings → LLM. Replace with: (a) `provider` dropdown bound to auto-detected binaries (`claude` ✓ / `codex` ✓ / unavailable ✗), (b) profile directory text input (injected as `CLAUDE_CONFIG_DIR` or `CODEX_HOME` in subprocess env, depending on provider), (c) `model` dropdown. Add `LLMConfig.config_dir` field. Wire env-var injection in both `ClaudeCodeCLIProvider` and `CodexCLIProvider`; pass `--skip-git-repo-check` to `codex exec` | (a) keep `cli_command` + add validation; (b) remove `cli_command` entirely with per-provider env-var wiring (this option); (c) keep `cli_command` but make it a dropdown of auto-detected binaries | Removing the input eliminates the alias/shell-function class of bug at the source. The user's actual need — an isolated profile — is served by env-var injection: `CLAUDE_CONFIG_DIR` for claude, `CODEX_HOME` for codex (verified working empirically on the maintainer's machine, 2026-04-30). Implementation is symmetric across both providers — same pattern, just different variable names. Validation alone leaves the foot-gun in place. The dropdown alternative is essentially the same as the new `provider` dropdown. |
...
|---|----------|-------|-----------|
| OQ1 | ~~Verify codex `CODEX_HOME` compatibility~~ — **resolved 2026-04-30 by live test on maintainer's machine** (codex-cli 0.118.0). `CODEX_HOME` works. `CLAUDE_CONFIG_DIR` works for claude 2.1.123. Both providers can be implemented with env-var injection and a documented minimum version. | resolved | — |
| OQ1a | Document minimum supported versions in app docs (claude ≥ 2.1, codex ≥ 0.x with `CODEX_HOME` support — confirmed for 0.118.0). On preflight, optionally also check version output and warn if below minimum. Whether to ship the version-floor check now or defer is the remaining sub-question. | Maneesh | before /spec |
...
| 3 | F15: Live verification on maintainer's machine surfaced that **claude has no `--config-dir` flag** (env-var only), so my "claude uses flag, codex uses env" mental model was wrong — both are env-var driven. Also discovered `codex exec` requires `--skip-git-repo-check` outside a git repo. | Updated Problem #4 implementation note to reflect symmetric env-var injection (`CLAUDE_CONFIG_DIR` + `CODEX_HOME`). Updated D9 to drop "wire `--config-dir`" wording. Resolved OQ1 (env vars verified working on claude 2.1.123 + codex-cli 0.118.0). Added the `--skip-git-repo-check` requirement for `CodexCLIProvider`. Adjusted wireframe 07 help text in two states. |
