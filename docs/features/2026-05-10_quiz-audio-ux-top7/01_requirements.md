...
| FR-WPM-CONFIG | TWO net-new config fields (resolves MSF S1 wpm asymmetry): `Settings.audio.tts.listen_wpm` (default 200, range 100–400, step 25) drives the listen-time formula `sum(word_count) ÷ listen_wpm`; `Settings.reading.reading_wpm` (default 250, range 100–500, step 25) drives the chapter-picker reading-time estimate (Q-7 fix). Both are independently configurable via Settings panels and env vars. **/spec inspection (2026-05-10) confirmed:** no `wpm` field exists on `TTSConfig` today, and no `ReadingConfig` block exists — both are purely additive (no rename / no backward-compat shim required). Persisted via existing `PATCH /api/v1/settings` → `settings.yaml` round-trip. | Settings → TTS panel + new Settings → Reading panel; `GenerateAudioModal.vue`; `ScopePicker.vue`; `backend/app/config.py` TTSConfig + new ReadingConfig | D3 (listen-time formula); Q-7 (reading-time); MSF S1 |
