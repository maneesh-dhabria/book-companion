...
| FR-14 | Modal close-X icon button (resolves REVIEW-LOG: existing `role="dialog"` + `aria-modal="true"` + `aria-labelledby="gen-audio-title"` already present — add only the close-X). Close-X has `aria-label="Close generate audio dialog"` and triggers `emit('close')`. | DOM assertion: button visible top-right with aria-label; click triggers close. |
...
| FR | Description | Acceptance |
|----|-------------|------------|
| FR-16 (FR-WPM-CONFIG split) | `SettingsTtsPanel.vue` mounts a labelled slider for `audio.tts.listen_wpm` (range 100–400, step 25, default 200). New: `SettingsReadingPanel.vue` (or new section in existing settings) mounts a labelled slider for `reading.reading_wpm` (range 100–500, step 25, default 250). Both PATCH `/api/v1/settings` on commit. | Vitest: render + drag slider; assert PATCH body. E2E: change values, assert Generate-modal Y and ScopePicker reading-time recompute. |
