...
| D8 | Stderr capture window in `claude_cli.py` — keep last 500 bytes of stderr stream (`STDERR_TRUNCATE = 500` in claude_cli.py — code-confirmed during /execute T4) as `stderr_truncated` (existing `STDERR_TRUNCATE` const) and pass it through to the route handler so the 502 detail body carries actionable diagnostic text. | (a) full stderr (PII risk on long traces); (b) first 500 B; (c) **last 500 B (chosen — matches existing constant)**; (d) no truncation. | Last 500 B (existing constant) captures the failure tail (where Python tracebacks land their final frame). Existing exception already has `stderr_truncated` plumbing — reuse, don't add new field. |
...
| D12 | wpm storage: ADD `Settings.tts.listen_wpm` (default 200) and `Settings.reading.reading_wpm` (default 250). **Code-study during /execute T5 confirmed:** `Settings` has `tts:` at root (no `audio.` wrapper). Both fields are net-new — no legacy `wpm` existed. Env vars: `BOOKCOMPANION_TTS__LISTEN_WPM`, `BOOKCOMPANION_READING__READING_WPM`. | (a) keep single `wpm`; (b) two new fields, drop legacy; (c) **rename + add + back-compat read (chosen)**; (d) move both under a new `wpm.*` namespace. | (a) leaves S1 asymmetry. (b) breaks any existing user override. (c) ships safely on a single-user tool; (d) over-engineers a 2-field migration. |
...
No legacy `wpm` field shipped previously — both `tts.listen_wpm` and `reading.reading_wpm` are net-new. No deprecation needed (verified during /execute T5).
