...
| FR-04 | `useTtsPlayerStore.nextSentence()` / `prevSentence()` MUST delegate to `nextActive()` / `prevActive()` WITHOUT pre-updating `sentenceIndex`. The engine's `onSentenceChange` callback is the sole source of truth for `sentenceIndex` updates — this prevents store/engine drift when the engine bounds out at the last/first sentence (covers B-01). The same rule applies to `seekActive(idx)` — store does not pre-set the index. |
...
| FR-18 | `engine.pause()`, `engine.nextSentence()`, `engine.prevSentence()`, `engine.seek(idx)`, and `engine.terminate()` MUST cancel any pending voice-wait listener + timeout AND any active watchdog before delegating to the underlying synth/audio API. This guarantees no stale timers fire after the user pauses, navigates, seeks, or closes during a `'starting'` state (covers E3, E5, E6, B-03, B-04). The engine SHOULD hold a single `voiceWaitCleanup: () => void` reference + `watchdogTimer: number | null` so cleanup is idempotent. |
...
| FR-20 | When `useTtsEngine.playActive()` is invoked while `lastEngine === null` (preload still in flight, or `load()` failed), it MUST call `store.setError('engine_unavailable')` instead of silently no-op'ing. This closes the original-bug class for the null-engine race (covers B-02). |
| FR-21 | `OpenContentArgs` MUST gain a `bookId: number` field, and `useTtsPlayerStore` MUST capture it as a ref so it can be supplied to a re-issued `engine.load()`. |
| FR-22 | `store.retry()` MUST, after invoking `open(...)`, call `useTtsEngine().load({ bookId, contentType, contentId, sentenceIndex: at })` so the Retry button actually re-attempts engine load (covers B-06 + B-09). |
| FR-23 | `WebSpeechEngine.play()` MUST be idempotent against repeated invocations: if `this.currentUtterance` is non-null and the utterance has not yet ended, the call MUST be a no-op. `Mp3Engine.play()` MUST be idempotent equivalently: if `audio.paused === false` and `audio.ended === false`, the call MUST be a no-op (covers B-05 rapid double-click). |
| FR-24 | `store.close()` MUST call `useTtsEngine().terminate()` before clearing local state, so the engine + its timers are cleanly torn down on Playbar dismiss (covers W-01). |
| FR-25 | `WebSpeechEngine`'s `utterance.onstart` handler MUST check whether the engine has already emitted an error (e.g., from a prior watchdog timeout). If so, it MUST call `synth.cancel()` to silence the late-arriving utterance and keep audio in sync with the visible error state (covers B-08). |
