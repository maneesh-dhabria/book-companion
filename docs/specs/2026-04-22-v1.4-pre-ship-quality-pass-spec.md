...
| FR-A1.2 | `PresetPicker.vue` calls the new endpoint on mount and renders cards from the response. Remove hardcoded `presets` array (`PresetPicker.vue:16-25`). The fetch is NOT cached across mounts — each `PresetPicker` mount, each `PresetPickerModal` open, and each `BookSummaryView` Generate-button click fires a fresh GET (endpoint is <50ms for 5 files per NFR-01; caching is not worth the staleness risk on a personal tool where users may add YAML files). **Unified fetch-failure UI (W-1 resolution):** all three consumer surfaces use the same inline error component `<PresetsFetchError @retry>` that renders "Couldn't load presets" + a Retry button, and blocks submit until presets are loaded. |
...
| FR-A1.4 | **LLM provider availability endpoint.** New `GET /api/v1/llm/status` returns `{"available": true|false, "provider": "claude"|"codex"|null, "version": string|null}`. `available=false` means neither `claude` nor `codex` is on `$PATH`. Upload wizard calls this on mount; when `available=false`, renders a persistent banner "Summarization requires Claude or Codex CLI — [install guide](#)" and disables the Start Processing button. `BookSummaryView` Generate button is disabled on the same condition. Resolves S-5. |
...
| FR-A4.2 | Full stderr is appended to `{BOOKCOMPANION_DATA_DIR}/summarization-failures.log` via `RotatingFileHandler(5MB, backupCount=3)`. **Format (B-4 resolution):** plain text, one line per entry: `ISO_TIMESTAMP | section_id=N | book_id=N | type=CODE | stderr_truncated_to_10k_chars`. Greppable via `grep`, `awk`, `tail -f`. JSON-lines format is deferred (v1.5+) if structured analysis becomes necessary. |
...
| FR-A4.5 | **Subprocess timeout wrapper.** `ClaudeCodeCLIProvider._run_subprocess` (and `CodexCLIProvider`) wraps the subprocess call in `asyncio.wait_for(proc.communicate(), timeout=settings.llm.timeout_seconds)` (default 300s per existing config). On timeout, the handler calls `proc.kill()` and raises `SubprocessTimeout`, which the caller maps to failure type `cli_timeout` per FR-A4.1. Makes the `cli_timeout` failure type in §10.1 live rather than a dead letter. Resolves S-13. |
| FR-A5.0 | `SummarizerService.summarize_section()` records the preset on `BookSection.last_preset_used` AND increments `attempt_count` BEFORE invoking the LLM subprocess, in a single `UPDATE book_sections SET last_preset_used = :preset, attempt_count = attempt_count + 1, last_attempted_at = :now WHERE id = :id` statement (SQL-side atomic; prevents TOCTOU per P-1). The update is committed before subprocess to ensure that even if the subprocess crashes the Python process, Retry (FR-A5.1) has the original preset to pre-select and the counter is accurate. |
...
| FR-A7.3 | **Explicit in-flight job check.** Before creating a new `ProcessingJob` via `POST /books/:id/summarize` or `POST /books/:id/book-summary`, the handler checks for an existing `ProcessingJob` where `book_id = :id AND status IN ('PENDING', 'RUNNING')`. If found, returns 409 with `{"detail": "A summarization job is already running for this book"}`. Belt-and-suspenders with FR-A7.6 (DB constraint). Resolves S-4. |
| FR-A7.4 | **Orphan job sweep at startup.** During server startup, sweep `ProcessingJob` rows where `status IN ('PENDING', 'RUNNING')`. For each, check liveness: if the row has a `pid` column populated, use `os.kill(pid, 0)` to check whether the process exists; if `pid` is NULL or the process doesn't exist, mark the job `FAILED` with `error_message='Server restarted mid-job'`, set `completed_at=utcnow()`. Log counts at info level. This prevents stuck RUNNING jobs after server crash/restart. Resolves S-8. |
| FR-A7.5 | **SSE reconnect reconciliation.** Frontend consumers of `/processing/:job_id/stream` are store-idempotent: replaying an already-seen `section_completed` or `section_failed` event (identified by `section_id`) is a no-op, not a double-increment. On `EventSource.onerror`, the frontend reconnects automatically (default `EventSource` behavior). Backend does NOT replay buffered events. If the frontend suspects missed events (e.g., gap in `index` field across events), it fetches `GET /processing/:job_id` to reconcile aggregate state (completed/failed/skipped counts). Resolves S-9. |
| FR-A7.6 | **DB-enforced one-active-job-per-book invariant.** Add SQLite partial UNIQUE index: `CREATE UNIQUE INDEX ix_processing_jobs_one_active_per_book ON processing_jobs(book_id) WHERE status IN ('PENDING','RUNNING');`. This closes the TOCTOU race in FR-A7.3 — if two near-simultaneous POSTs both pass the application-level check, the second INSERT raises `IntegrityError`, which the handler catches and returns as the same 409. Resolves P-5. |
...
| FR-B1.3 | Route param watcher (`BookDetailView.vue:72`) retains current behavior for in-app navigation (no regression). The same watcher handles browser Back navigation — Vue Router's back-button triggers the route-param change, which triggers the watcher, which calls `loadFromRoute` → `loadBook` per FR-B1.1. No separate back-button spec needed. Resolves B-14. |
...
| FR-C1.2 | Generate button opens `PresetPickerModal` with the last-used section preset pre-selected (D11). If no section summaries exist, button is disabled with tooltip "Summarize sections first" (D16). Immediately on click (before the POST round-trip), the button is disabled to prevent double-POST (S-10 resolution); it re-enables on POST response (success or failure). |
...
| FR-C1.7 | **Book-summary job failure storage + display.** Book-summary job failures write the truncated error (≤500 chars) to `ProcessingJob.error_message` (existing column; no new `Book` columns). When `ProcessingJob.status='FAILED'` for a book-summary job (i.e., the job whose id matches the one returned by `POST /books/:id/book-summary`), `BookSummaryView` renders an inline failure banner with the stored message + Retry CTA that re-opens the preset-picker modal. Resolves S-2. |
| FR-C1.8 | **Book-summary post-success wiring.** On successful completion of a book-summary LLM call, the handler (a) INSERTs a `Summary` row with `content_type='book', content_id=book.id, summary_md=<llm_output>`, (b) SETs `Book.default_summary_id` to the new Summary's id, (c) COMMITs, (d) publishes `processing_completed` SSE with `book_summary_id=<new summary id>`. If any of (a)-(c) fails, the job is marked FAILED per FR-C1.7. Resolves B-12. |
| FR-C4.3 | **Clearing failure state on successful summarization.** On successful completion of `summarize_section()` (including retry), the handler sets `BookSection.last_failure_type = NULL` and `BookSection.last_failure_message = NULL`. `attempt_count` is NOT reset (it remains a cumulative counter for diagnostics). `last_preset_used` is NOT cleared (it reflects the preset of the successful summary). Without this, the UI's failure banner would persist after retry success. Resolves B-11. |
...
| FR-D1.4 | **Chat-send optimistic-update rollback.** `aiThreadsStore.send(content, scope)` optimistically appends the user message to `store.currentThread.messages` BEFORE awaiting the POST response. On success, it refreshes the thread (which replaces the optimistic message with the server's version + assistant reply). On POST failure, it REMOVES the optimistic message and surfaces the error inline (error banner in `AIChatTab.vue`, the message content preserved in the input for user to retry). Today's store has no error path; this formalizes it. Resolves B-15. |
| FR-D1.5 | **Reader initial-load state.** While `loadBook(bookId)` is awaiting the parallel `GET /books/:id` + `GET /books/:id/sections` calls, `BookDetailView.vue` renders a centered spinner in the reading-area (reuse existing loading-skeleton pattern if one exists in `LibraryView`; otherwise a minimal `<LoadingSpinner />` component). No blank white page. Resolves B-13. |
...
| FR-D3.3 | Scope selector **default-on-mount** rule: Section when the user first opens a reader view, Book when on `/books/:id/summary`. Once the user explicitly changes the scope, **the choice persists for that thread** for the rest of the session (stored on the thread object in memory). Opening a different thread resets to the default-on-mount rule. Resolves S-16. |
...
| FR-F3.2 | **Zero-section EPUB handling.** If `POST /books/upload` parsing yields 0 `BookSection` rows, the upload wizard's Structure step renders an error banner: "No sections detected in this file. The file may be malformed, empty, or use an unsupported EPUB flavor. Try a different file." The Back button returns to Upload step; the Continue button is disabled. Prevents progression with broken input. Resolves S-14. |
...
| NFR-10 | Contract | SSE consumers are **store-idempotent**: a replayed `section_completed`/`section_failed`/`section_skipped` (same `section_id`, same event type) produces no state change beyond the first occurrence. Implementation detail: the `summarizationJob` Pinia store tracks processed `section_id` sets per event type and short-circuits duplicates. |
...
**`default_id` rule (FR-A1.1 extension, S-5 resolution):** Always the string `"practitioner_bullets"` in v1.4 (matches the existing CLI default). Not configurable in v1.4; if the named default is absent from the directory, `default_id` is the first preset in the sorted array.

**Fault tolerance (S-15 resolution):** `PresetService.list_all()` catches per-file parse errors (malformed YAML, missing required keys like `facets`), logs `WARN` with the filename and error, and skips the bad file. Returns the list of successfully-parsed presets. The endpoint does NOT 500 the whole request because of one broken file.

**Response (500):** `{"detail": "Could not load presets"}` only if the YAML directory itself is unreadable (permission denied on the dir). Should not occur in practice.
...
- `404` — `{"detail": "Book {book_id} not found"}` — for unknown book_id (B-6 resolution).
...
**SSE event applicability per job type (B-7 resolution):**

| Event | section jobs (scope=section, book, failed) | book-summary jobs (POST /book-summary) |
|-------|--------|--------|
| `processing_started` | ✓ | ✓ |
| `section_started` | ✓ (one per summarizable section) | ✗ (no sections involved) |
| `section_completed` | ✓ | ✗ |
| `section_failed` | ✓ | ✗ |
| `section_skipped` | ✓ (for already-summarized) | ✗ |
| `section_retrying` | ✓ (eval auto-retry path) | ✗ |
| `processing_completed` | ✓ (with `completed/failed/skipped`) | ✓ (with `book_summary_id`) |
| `processing_failed` | ✓ | ✓ (per FR-C1.7) |

Frontend consumers of `/processing/:job_id/stream` use `job.scope` (fetched via `GET /processing/:job_id` on SSE open) to know which event set to expect.
...
### 9.6 `GET /api/v1/llm/status` (new — FR-A1.4)

New endpoint for detecting LLM-CLI availability without attempting a summarize and catching the failure.
...
Or, when neither CLI is installed:
```json
{
  "available": false,
  "provider": null,
...
Detection follows the existing `detect_llm_provider()` logic (`shutil.which()` for `claude` then `codex`). Version is best-effort: invoke `claude --version` / `codex --version` with a short timeout and parse the first line; if parsing fails, `version=null` but `available=true` provided the binary exists.
...
-- FR-A7.4 orphan-sweep support
ALTER TABLE processing_jobs ADD COLUMN pid INTEGER;

-- FR-A7.6 DB-enforced one-active-job-per-book invariant
CREATE UNIQUE INDEX ix_processing_jobs_one_active_per_book
  ON processing_jobs(book_id) WHERE status IN ('PENDING','RUNNING');
```

**Failure type enum values (application-layer, stored as VARCHAR for flexibility):**
- `cli_nonzero_exit` — subprocess returned non-zero (including segfault rc=139)
- `cli_not_found` — `claude` / `codex` binary not on `$PATH` at subprocess spawn time
- `cli_timeout` — subprocess hit the configured timeout (default 300s, FR-A4.5)
...
- For every `Summary` row where `summary_md IS NULL OR TRIM(summary_md) = ''`:
  ...
  - UPDATE BookSection SET `last_failure_type='empty_output', last_failure_message='Cleaned up empty summary from prior run', last_attempted_at=summary.created_at, attempt_count=attempt_count+1` **WHERE section.id = :section_id AND (last_failure_type IS NULL OR last_failure_type = 'empty_output')** — guard clause prevents overwriting a more-recent non-empty-output failure on the same section (P-3 resolution).
