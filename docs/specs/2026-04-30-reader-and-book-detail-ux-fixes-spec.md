...
| FR-12 | The view MUST handle three render modes: (a) active (status PENDING/RUNNING) → progress bar, current section, ETA, [Open Book] [Cancel]; (b) terminal (COMPLETED/FAILED) → status card with summary stats and [Open Book]; (c) 404 → "Job not found — it may have been deleted" with [Back to Library]. |
...
| FR-24 | The static header counter line ("Summaries: X of Y (Z pending, W failed)") MUST be removed. `SummarizationProgress.vue` (already mounted below the action row) is the single source of truth and remains. |
| FR-25 | `onExportClick` MUST set a local `exporting` ref to `true` before the fetch and to `false` after a minimum 250ms has elapsed AND the response has arrived. Implementation pattern: `await Promise.all([exportPromise, new Promise(r => setTimeout(r, 250))])`. The Export button MUST show a spinner while `exporting === true`. |
...
| FR-32 | `SectionBrief` MUST gain a `content_char_count: int` field served by the book-detail and section-list endpoints. Backend computes via `len(content_md or "")` at serialize time — no new schema column. Personal-scale library (< 100 books, < 5k sections per book) makes the read-time derivation cost negligible. |
...
| NFR-06 | Migration safety | Both new data migrations (FR-07 image-rewrite, FR-34 preset typography) MUST be idempotent — re-running produces zero UPDATEs. Verified by integration tests in §12.3. |
