...
| FR-05 | Kokoro model files live at `<data_dir>/models/tts/{kokoro-v1.0.onnx, voices-v1.0.bin}`. Download is implicit on first `synthesize()` call (D31); `huggingface_hub.hf_hub_download(...)` with `cache_dir=<data_dir>/models/tts`. Console banner: `"Downloading TTS voice model (first time only, ~80MB)..."`. |
...
| FR-10 | Sentence-segment the sanitized text via `pysbd.Segmenter(language="en", clean=False).segment(text)` (D17). Compute char-offsets per sentence start as the second return value. |
...
| FR-21 | Per-content-unit progress also written to `ProcessingJob.progress` JSON dict: `{completed, total, current_kind, current_ref}` so reconnecting clients see latest state. |
...
| FR-24 | `DELETE /api/v1/books/{id}/audio` (NEW): delete all `AudioFile` rows + unlink files for the book. Type-to-confirm in UI per `x-interaction.destructiveActions`. |
| FR-25 | `DELETE /api/v1/books/{id}/audio/{content_type}/{content_id}` (NEW): per-row delete (per MSF N1). UI: trash icon on Audio tab + Sections tab rows. |
...
| ID | Requirement |
|----|-------------|
| FR-28 | New `frontend/src/stores/ttsPlayer.ts` Pinia store: `{isActive, contentType, contentId, sentenceIndex, totalSentences, status: 'idle'|'loading'|'playing'|'paused', engine, voice, mp3Url|null, sentenceOffsets[], stale: bool, mediaSessionEnabled: bool}`. |
...
| FR-32 | Player chrome shows "Limited controls" pill + tooltip when `engine === "web-speech"` (D26). Tooltip text: "Lock-screen controls and offline playback require pre-generating audio. Generate from the Audio tab." |
...
| FR-43 | `GET/PUT /api/v1/settings/tts` (NEW; or extends existing `/api/v1/settings` PATCH). Settings UI (file `07_settings-tts_*.html`) reads via `SettingsService`, writes via validated `update_settings()`. |
...
| NFR-09 | Cross-browser — playback | Chrome/Safari macOS desktop + Safari iOS support both engines for tab-active playback; lock-screen requires Kokoro+`<audio>` (D34 / FR-33) |
...
| NFR-12 | Logging | Audio job events logged via `structlog` with `book_id, job_id, kind, ref, voice, engine` keys |
...
```
**Errors:** `400` invalid scope/voice/engine (engine="web-speech" not allowed for pre-gen); `404` book not found; `409` audio job already PENDING/RUNNING for this book; `503` ffmpeg missing or Kokoro model not downloadable.
...
```
When `pregenerated == false`, the response omits `url`, `duration_seconds`, `voice`, `sentence_offsets_seconds`, `source_hash_stored`, and `stale` (set to `null`); the frontend uses `sanitized_text` + `sentence_offsets_chars` to drive the Web Speech path.

Backend logic for `stale`: sanitize current source → SHA-256 → compare to `AudioFile.source_hash`. Equal → `false`; different → `true`. Implemented in `AudioGenService.lookup()`.
...
- Rollback: provided as `downgrade()` in the Alembic revision (recreate old index, drop new tables).
