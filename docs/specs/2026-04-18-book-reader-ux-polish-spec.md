...
| FR-02 | Add new title-regex patterns to `SECTION_TYPE_PATTERNS` (order matters; first match wins): `copyright` → `r"\bcopyright\b|©"`, `acknowledgments` → `r"\backnowledg(e)?ments?\b"`, `dedication` → `r"\bdedication\b"`, `title_page` → `r"\btitle\s*page\b"`, `table_of_contents` → `r"\b(table\s+of\s+contents|\bcontents\b)"`, `colophon` → `r"\bcolophon\b"`, `cover` → `r"^cover$|^book\s*cover$"` (anchored — "Discover" must not match), `part_header` → `r"^part\s+(one|two|three|four|five|six|seven|eight|nine|ten|\d+)\b"`. These patterns are evaluated before the existing patterns so front-matter is detected before body-content keywords like "introduction". |
| FR-03 | `detect_section_type(title, content_md)` takes a second arg `content_md: str | None`. If a title matches the `part_header` pattern AND `len(strip_markdown(content_md)) < 1000`, returns `"part_header"`. If the title matches `part_header` but content is ≥ 1000 chars (real prose), return `"chapter"` instead (content-aware gate). Other patterns unchanged. |
...
| ID | Requirement |
|----|-------------|
| FR-13 | Create a new Alembic revision `<hash>_reclassify_sections_and_prune_frontmatter.py`. In `upgrade()`: for each row in `book_sections`, re-run `detect_section_type(title, content_md)` and UPDATE `section_type` where the computed type differs from the stored value. |
...
| FR-15 | The migration tracks counters: `reclassified`, `auto_summaries_pruned`, `user_summaries_preserved`, `books_affected`. Logs the totals via `print()` (visible during `serve` startup since migrations run synchronously at the top of `init`). |
...
| FR-31 | `reader.ts` action `loadBook(bookId)` — when no saved reading-state exists, set `currentSection = sections.find(s => SUMMARIZABLE_TYPES.has(s.section_type)) ?? sections[0]` instead of `sections[0]`. Existing last-read-position restoration from `useReadingState()` takes precedence if a position exists. |
...
| FR-33 | New component `frontend/src/components/reader/SummaryEmptyState.vue` accepts props `{section: Section, activeJobSectionId: number | null}`. Renders: (a) front-matter → "Summary not applicable for {title}" (static text, no CTA); (b) non-front-matter without summary, not being processed → "Not yet summarized" heading + *Summarize this section* button; (c) this section is active in a running job (`activeJobSectionId === section.id`) → "Generating summary…" with subtle spinner. |
...
| FR-38 | New Pinia store `stores/summarizationJob.ts` tracks `{bookId: number, jobId: number, activeJobSectionId: number | null, scope: 'all'|'pending'|'section'}` for the current job. Actions: `startJob(bookId, params)` (POSTs to summarize endpoint, opens SSE via existing `connectSSE`), `onSectionStarted(sid)` / `onSectionCompleted(sid)` / `onCompleted()` / `onFailed(err)` handlers, `reset()` clears all state. Exposes `isActive` + `activeJobSectionId` computeds consumed by `SummaryEmptyState` (FR-33) and `SummarizationProgress` (FR-36). BookDetailView calls `summarizationJob.reset()` from an `onBeforeRouteLeave` router guard so leaving the page closes the EventSource and clears state; Pinia store itself is a singleton but the per-book lifecycle is owned by the view. |
| FR-39 | On `section_completed(section_id=N)`, the store refetches `GET /books/{bookId}/sections/{N}?include_content=true` and calls a new `reader.updateSection(section)` action that splices the new row into `reader.sections` at matching index AND updates `reader.currentSection` if it's the same section. This keeps the TOC's "S" marker (`has_summary`) live AND the Summary tab's rendered content fresh, without a full book refetch. |
| FR-39a | **Frontend API-client mirror** (`frontend/src/api/processing.ts`): extend `ProcessingOptions` interface with `scope?: 'all' | 'pending' | 'section'` and `section_id?: number`. Add a new handler slot to `SSEHandlers` — `onProcessingStarted?: (data: { book_id: number; job_id: number; total_sections: number; scope: string }) => void` — and register its listener in `connectSSE` analogously to the existing events. `summarizationJob` store (FR-38) wires `onProcessingStarted` to clear any stale state and flip `isActive = true` before the first `section_started` arrives. |
...
| E4 | SSE connection drops mid-job | Network glitch / tab backgrounded | `connectSSE`'s `onerror` fires. `summarizationJob` store sets `jobId = null`, but does NOT mark work as failed (the server-side job continues). On next refetch of the book, `summary_progress` reflects actual state. |
