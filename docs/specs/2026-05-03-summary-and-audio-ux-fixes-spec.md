...
| FR-14 | `AppShell.vue` adds a global `keydown` listener that toggles `ttsPlayer.play()`/`pause()` when `Space` is pressed and `ttsPlayer.isActive===true` and the active element is NOT an `<input>`/`<textarea>`/`[contenteditable]`. Existing `ReadingArea.vue:45` handler stays (idempotent). |
...
| FR-17 | When Web Speech is unsupported (`!('speechSynthesis' in window)`) AND the lookup will not have a pre-generated MP3, the `<TtsPlayButton>` is **not rendered**. Detection happens at component mount: a one-shot probe `'speechSynthesis' in window` cached in a small composable `useWebSpeechSupported()`. If the per-content MP3-existence is unknown at render time (it is — lookup is on click), the button is rendered when Web Speech is supported, else hidden. (When Web Speech is unsupported, the user falls back to the audio-tab pre-generation path.) |
...
| FR-25a | `<TtsPlayButton>` preloads its lookup on `onMounted` (debounced 500ms per surface — share a singleton lock keyed by `${contentType}:${contentId}` so duplicate buttons in the same render coalesce). Result is cached in a small in-memory map keyed by `${contentType}:${contentId}`. Click handler reads from cache (or awaits the in-flight preload promise) and calls `engine.play()` synchronously to preserve iOS Safari's user-gesture chain (D16). Cache TTL: 5 minutes — long enough for a session, short enough that a re-mounted view re-checks staleness. The cache is invalidated on `processing_completed` SSE for any content (audio re-gen) and on book/section delete. |
