...
| FR-03 | The `state==='inProgress'` template renders: an indeterminate spinner element (CSS `@keyframes` rotation, respecting `prefers-reduced-motion`) and the text `Generating book summary… {{ formatElapsed(elapsedMs) }} elapsed`. Format: `M:SS` for `< 10:00`, `MM:SS` thereafter. Initial render reads `0:00`. |
...
| FR-07 | On 409 response, the body's `active_job_id` is attached via the existing `attachSse(jobId)` path, AND `startedAt` is set to `Date.now()` (so the elapsed timer starts at "0:00 elapsed" — we don't know the real start; this is acceptable and called out in §12 E1). |
...
| FR-14 | `AppShell.vue` adds a global `keydown` listener that toggles `ttsPlayer.play()`/`pause()` when `Space` is pressed and `ttsPlayer.isActive===true` and the active element is NOT an `<input>`/`<textarea>`/`[contenteditable]`. The Space branch in `ReadingArea.vue:31-54` is **removed** (FR-14b) so the global handler is the single owner — without removal, both handlers fire and cancel each other (no-op race). ArrowLeft/ArrowRight branches in ReadingArea stay (they're reader-scoped sentence-skip, not global). |
...
| FR-15 | A `<TtsPlayButton>` appears as the **leftmost** item in the header action row of: `BookSummaryTab.vue` (when `state==='populated'`), `SectionSummaryTab.vue` (or equivalent — see §8.3 audit), and `SectionContent.vue` / `SectionDetailView.vue`. Bound to `(contentType, contentId)` per surface — see §7.3. |
...
| FR-17 | `<TtsPlayButton>` always renders initially (so the preload in FR-25a can fire). Its visibility is then computed: `visible = webSpeechSupported \|\| preloadResult?.pregenerated === true`. While `preloadResult === undefined` (preload in flight), the button renders with `visibility: hidden` (occupying layout) for up to 300ms; after 300ms or once the preload resolves, visibility flips to its final state. This keeps Web-Speech-unsupported users on the MP3 path and prevents layout-thrash on slow networks. (`useWebSpeechSupported()` composable still caches the one-shot Web Speech probe.) |
...
| FR-25a | `<TtsPlayButton>` preloads its lookup via an `IntersectionObserver` (root: viewport; rootMargin: `200px`) — fires when scrolled into view, debounced 500ms after intersection. Per-content singleton lock keyed by `${contentType}:${contentId}` coalesces duplicate buttons in the same render. Result cached in a small singleton map keyed by `${contentType}:${contentId}`. Click handler reads from cache (or awaits the in-flight preload promise) and calls `engine.play()` synchronously to preserve iOS Safari's user-gesture chain (D16). Cache TTL: 5 minutes (backstop). Cache invalidation: subscribe to the existing `audioJob` Pinia store events (`section_audio_completed`, `book_audio_completed`, `section_audio_failed`, etc. — see CLAUDE.md gotcha #23 for the canonical event names); on any audio-completion event, evict the matching `${contentType}:${contentId}` key. Non-audio SSE events are ignored. |
...
- Empty annotation (no `selected_text`, no `note`): returns the empty shape (`pregenerated:false`, `sanitized_text:""`, `sentence_offsets_chars:[]`) — caller's choice whether to render the Listen button (Q3).
...
| E12 | `processing_jobs` partial UNIQUE collision | User clicks Generate-summary twice in same second | The existing partial UNIQUE index on `(book_id) WHERE status IN ('PENDING','RUNNING')` rejects the second INSERT → API returns 409 → E1 reattach. No new code needed. |
...
|----|----------|-------|-----------|
| Q1 | _(Resolved by D16 / FR-25a — preload-on-mount eliminates the gesture-chain dependency.)_ | — | — |
...
| Q3 | Listen button on a totally-empty annotation (no `selected_text`, no `note`) — hide entirely (per E4 alt) or render and let it open Playbar that immediately ends? | Maneesh | Before /plan finalizes |
