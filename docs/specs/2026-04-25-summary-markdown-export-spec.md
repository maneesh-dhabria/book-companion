...
| # | Goal | Success Metric |
|---|------|---------------|
| G1 | One-click default export from book detail page; bounded render time | `Export summary` button reachable in ≤ 1 click from `/books/:id`; produces full export with default selections; server-side render < 500ms for a 30-section book and ≤ 5s for books up to 100 sections / 1000 annotations (linear scaling) |
...
| D21 | Slug functions diverge by purpose: TOC anchor slug (FR-B7) is **Unicode-aware** to match GFM viewer auto-anchors; filename slug (FR-A2) is **ASCII-only** for cross-OS filesystem safety. Two separate helpers, one intent each. | (a) **Diverge by purpose** (chosen), (b) Single shared helper (would either lose CJK in TOC anchors or pull `unidecode` into filename derivation), (c) Library-based (`python-slugify` for both — adds dep + transliteration ambiguity). | TOC anchors must round-trip through GFM auto-anchor matching, which is Unicode-aware in GitHub / Obsidian / VS Code. Filenames must work everywhere — including legacy filesystems and shells where non-ASCII path handling is fragile. The two contexts have different correctness criteria; one helper would force a wrong choice for one of them. |
...
| ID | Requirement |
|----|-------------|
| FR-B1 | Add `_render_summary_markdown(book_data: dict, selection: ExportSelection) → tuple[str, bool]` that produces `(body, is_empty)`. `is_empty = not (toc_emitted or book_summary_block_emitted or any_section_block_emitted or notes_block_emitted)` — derived from renderer intent (track these four block-emit booleans inside the function), NOT by text-grepping `body`. The route handler reads `is_empty` to set the `X-Empty-Export` response header (FR-A6). |
...
| FR-B5 | Image-URL sanitizer: zero `/api/v1/images/` substrings may survive in output. Implementation MUST apply these two regexes, in this order, to every `Summary.summary_md` before rendering: <br>(1) `!\[([^\]]*)\]\(/api/v1/images/\d+(?:\s+"[^"]*")?\)` → `[Image: $1]` if `$1` non-empty, else `[Image]`. The optional ` "title"` group accommodates the GFM image-with-title syntax `![alt](url "title text")`; the title is dropped — the placeholder uses only the alt text. <br>(2) `<img\s+[^>]*src=["']/api/v1/images/\d+["'][^>]*>` → `[Image: <alt>]` derived from the tag's `alt="..."` attribute, or `[Image]` if `alt` is absent / empty. <br>Order matters because (1) does not match raw `<img>` tags. The placeholder text uses verbatim alt text — even if the alt contains `[`, `]`, or parentheses, the resulting `[Image: ...]` renders as literal text in every common viewer (no Markdown link parsing risk because there's no following `(...)`). |
| FR-B6 | Markdown structure (top-to-bottom): <br>1. `# {Book.title}` (verbatim) <br>2. `**Author:** {name}` or `**Authors:** {name}, {name}` (per author count); `**Author:** Unknown` if zero authors <br>3. `**Exported:** YYYY-MM-DD` <br>4. (if `include_toc` AND at least one section will be rendered in step 6) `## Table of Contents` followed by a hierarchical list with `[Section title](#gfm-slug)` links; **the TOC alone reflects depth** via nested bullets (`BookSection.depth`). If no sections will be rendered (all excluded or none have summaries), the TOC heading and list are omitted entirely (`toc_emitted = false`) <br>5. (if `include_book_summary` AND `Book.default_summary_id` is set) book summary content directly (no extra heading) <br>6. For each section with a `default_summary_id` whose `id ∉ exclude_section_ids`, in `BookSection.order_index` ascending order: <br>&nbsp;&nbsp;`## {section.title}` — **flat H2 for every section regardless of `depth`** (the TOC's nested bullets convey hierarchy; section bodies stay at a single outline level so every section is a peer in document outlines / Obsidian sidebar). Anchor is the heading's GFM auto-slug; **no inline `<a id>` tag**. <br>&nbsp;&nbsp;{summary_md, sanitized per FR-B5} <br>&nbsp;&nbsp;(if `include_annotations` AND section has annotations with non-empty `selected_text`) `### Highlights` block (see FR-B8) <br>7. (if `include_annotations` AND there are free-floating annotations on this book — annotations with empty/null `selected_text`) `## Notes` footer (see FR-B9) |
| FR-B7 | TOC slug generation MUST follow GFM rules to align with viewer auto-anchors (GitHub web, VS Code preview, Obsidian, Marked-render variants): lowercase, drop characters that are not alphanumeric / space / hyphen / underscore, replace whitespace with `-`, collapse consecutive `-`. "Alphanumeric" is **Unicode-aware** (matches GFM): Latin-1 with diacritics, CJK ideographs, Cyrillic, etc. preserve through slugification — only symbols, emoji, and punctuation are stripped. Duplicate slugs disambiguate via GFM convention: first occurrence is bare slug `s`; subsequent occurrences append `-1`, `-2`, … (NOT `-2`, `-3`, …) in section-encounter order. **Empty-slug fallback:** if the GFM rules produce an empty string for a section title (purely non-alphanumeric — emoji-only, punctuation-only), the slug falls back to `section-{order_index}` (zero-padded to 3 digits, e.g., `section-007`). Fallback slugs feed the disambiguation table the same way as ordinary slugs. The same disambiguation table is used to compute the TOC link target — sections themselves emit only `## {title}` and rely on the viewer's auto-anchor matching the predicted slug. |
| FR-B8 | Each annotation under `### Highlights` renders as a Markdown blockquote of `selected_text`, optionally followed by an indented `> — Note: {note}` line if `note` is non-empty. Empty line between annotations. **Content escaping** (applies to both `selected_text` and `note`): newlines (`\r\n`, `\n`) collapse to a single space (multi-line annotations render on one line); leading block-level markdown triggers at the start of any rendered line — `>`, `#`, `-`, `*`, `+`, ordered-list markers like `1.` — are escaped with a leading `\` to prevent breaking out of the blockquote. The escape applies AFTER the `> ` prefix is added, so e.g. an annotation `> > nested` becomes `> \> > nested` on the line (visually `> > nested` once Markdown parses the `\>`). |
| FR-B9 | `## Notes` footer aggregates **book-scoped annotations** — `Annotation.content_type = BOOK_SUMMARY` only (the only book-level value of the three-member `ContentType` enum: `SECTION_CONTENT` / `SECTION_SUMMARY` / `BOOK_SUMMARY`). Partitioning rule for that subset: <br>• `note` non-empty AND `selected_text` empty/null → bullet `- {note}` <br>• `note` non-empty AND `selected_text` non-empty → bullet `- > "{selected_text}"\n  — {note}` (a blockquote of the highlighted passage with the note as a continuation line) <br>• `note` empty AND `selected_text` non-empty → bullet `- > "{selected_text}"` (book-level highlight without a note) <br>• both empty → silently skipped (defensive; should not normally exist) <br>If the rendered list would be empty after this partitioning, the `## Notes` heading is omitted entirely (no empty heading). Cascade rule (D10) does NOT apply to book-scoped annotations — they are book-level, not section-level, so they appear regardless of which sections are excluded. **Section-scoped annotations** (`content_type ∈ {SECTION_CONTENT, SECTION_SUMMARY}`) with empty `selected_text` are silently skipped from the section's `### Highlights` block (defensive — `note`-only section annotations have no anchor passage to render under). |
...
| FR-A2 | Filename header: `Content-Disposition: attachment; filename="{book-slug}-summary-{YYYYMMDD}.md"`. Filename slug is produced by a new helper `app/services/slug.py::filename_slug(title) → str`. Implementation: `re.sub(r"[^a-z0-9]+", "-", title.lower()).strip("-")[:80]` — **ASCII-only by design** (cross-OS filesystem safety, no RFC 5987 / `filename*=` encoding required, no transliteration ambiguity for CJK / Cyrillic / etc.). When the result is empty (CJK-only titles, emoji-only titles), fall back to `book-{id}-summary-{YYYYMMDD}.md`. For `format=json` the filename remains `book_{id}.json` (unchanged). The response also includes `Cache-Control: private, max-age=0` to prevent intermediary caching of personal export bytes. |
...
| FR-A6 | On `format=markdown`, the response carries header `X-Empty-Export: true` when the renderer's `is_empty` flag (FR-B1 tuple return) is `true` — i.e., none of the four block types (TOC, book summary, sections, Notes) was emitted. Else the header is omitted (or set to `false`). Detection MUST be intent-derived from the renderer, never inferred via text-grep on the body — book summaries containing literal `## ` substrings would otherwise produce false negatives. The frontend uses this header to pick the empty toast variant per FR-F12. The header is not set on `format=json` responses. |

### 6.3 CLI delta

| ID | Requirement |
|----|-------------|
| FR-C1 | `bookcompanion export book <id> --format markdown` accepts new flags: `--no-book-summary`, `--no-toc`, `--no-annotations`, `--exclude-section <id>` (Typer `multiple=True`). All default to "include" / empty. Each flag MUST have a Typer parameter `help="..."` string (visible in `bookcompanion export book --help`) explaining its effect in one sentence — e.g., `help="Exclude the book-level summary block from the Markdown export."`. The command's docstring lists the new flags under a `Flags only valid with --format markdown:` heading so `--help` output groups them coherently. |
...
| FR-C6 | A book with zero summaries anywhere AND `--format markdown` AND no `--no-*` flags produces a front-matter-only file (or stdout) with exit 0 — no warning, no error. CLI's "trust the user" ethos (D18 footgun rule applies to UI only). The CLI is also unaffected by `book.status` — invoking export on an `UPLOADING` or `PARSING` book proceeds with whatever `_collect_book_data()` finds (typically front-matter-only) and exits 0. The `BookStatus` gate (FR-F2) is UI-only; power users own the timing on the CLI. |
...
| FR-F5 | `Copy as Markdown` click writes the same fetched markdown to the clipboard. To survive Safari's user-gesture-chain rule (which rejects `clipboard.writeText` after an `await fetch`), the implementation MUST prefer the `ClipboardItem`-with-promise API and fall back to the legacy path only when unavailable: <br><pre>if (navigator.clipboard?.write && window.ClipboardItem) {<br>  await navigator.clipboard.write([new ClipboardItem({<br>    'text/plain': fetch(url).then(r => r.blob())<br>  })])<br>} else {<br>  const text = await fetch(url).then(r => r.text())<br>  await navigator.clipboard.writeText(text)<br>}</pre>**Toast disambiguation** on failure: <br>• If the inner `fetch()` rejected (TypeError, response.ok=false) — root cause is network — show the network toast (FR-F12: `Export failed — check your connection.`). <br>• If the fetch succeeded but `clipboard.write` / `clipboard.writeText` rejected with a permission `DOMException` (or `navigator.clipboard` is undefined) — show the clipboard toast (FR-F12: `Couldn't copy — try Export instead.`). <br>Implementation: catch the fetch separately from the clipboard call so the error type is unambiguous. Do NOT throw — toast is the only user-visible failure mode. |
| FR-F6 | `Customize…` click → open new `ExportCustomizeModal.vue` (reuses the structural pattern of `PresetPickerModal.vue`: fixed overlay, `.modal-body` width `min(560px, 92vw)`, Escape closes, `Export` + `Copy` + `Cancel` footer). **Focus management** mirrors `PresetPickerModal` precedent: first interactive control receives focus on mount; Tab / Shift-Tab cycles inside the modal; closing returns focus to the `Customize…` link that opened it. **On modal mount, issue `GET /api/v1/books/{id}` to refresh the cached section list against current server state** before the modal renders interactive controls. This prevents stale `exclude_section` IDs from a multi-tab section-mutation race; while the refresh is in flight, the modal shows a brief loading skeleton (no checkboxes interactable). If the refresh fails, fall back to the cached `book.sections` and surface a small inline notice ("Could not refresh — showing cached sections"). |
...
| FR-F13 | **Add `ToastContainer.vue`** (~30-line component) that renders `useUiStore().toasts` as a fixed-position stack at the top-right (or bottom-right) of the viewport, mounted once in `AppShell.vue`. Color/iconography per `Toast['type']` (success/error/info/warning). The container wraps the toast list in `role="status" aria-live="polite" aria-atomic="true"` so screen readers announce new toasts; individual toasts include `aria-label` describing the type ("success", "error", etc.). This closes the half-built toast infrastructure today. |
| FR-F14 | **In-flight UX:** clicking `Export summary` or `Copy as Markdown` (in either the book detail page or the Customize modal) sets a per-button `exporting: boolean` ref to `true`, applies `disabled` to BOTH primary buttons (Export and Copy together — they share the same fetch source), and renders an inline spinner inside the clicked button. The `exporting` flag resets to `false` in a `finally` block (regardless of success / failure / toast disambiguation). The Customize modal's `Export` and `Copy` footer buttons follow the same pattern. Prevents rapid double-click duplicate downloads, Export-then-Copy-mid-flight overlap, and dangling busy states on error. |
...
**Response (200, `format=json`):** unchanged — `JSONResponse` with full `_collect_book_data()` shape; selection params silently ignored. The JSON path is intentionally preserved for CLI/API parity (the CLI's `bookcompanion export book --format json` exists; users may also call this route directly via API for full-fidelity per-book backup), even though no v1.6 frontend surface consumes it.
...
| E27 | Section title is purely emoji or symbols (e.g., "🚀🎯") | GFM slug = empty after stripping non-alphanumeric | Slug falls back to `section-{order_index}` zero-padded to 3 digits (FR-B7). TOC link `[🚀🎯](#section-007)` resolves to the corresponding heading via the predicted slug. |
| E28 | Section summary contains GFM image with title attribute `![figure](/api/v1/images/17 "Caption")` | Per FR-B5 regex (1) extended | Renders as `[Image: figure]`; the title text is dropped. |
| E29 | Book summary text contains literal `## subhead` markdown | Should not flip emptiness | `X-Empty-Export` header reflects the renderer's `is_empty` flag (FR-A6 + FR-B1), not body text-grep. Header reads `false` because `book_summary_block_emitted = true`. |
| E30 | Book has BOOK_SUMMARY-scope annotations (highlights on the book summary itself) — common after annotating book overview | `content_type = BOOK_SUMMARY` rows present | Rendered in `## Notes` per FR-B9 partitioning rules: blockquote of selected_text, bullet of note, or both. |
| E31 | Section title contains Unicode (Café, 北京, Москва́) | Non-ASCII alphanumerics | Per FR-B7 Unicode-aware rule, characters preserve through slugification. Slug for `Café` → `café`; for `北京 概要` → `北京-概要`; matches GFM auto-anchor in viewers that support Unicode anchors (GitHub, Obsidian). |
