...
| D3 | Switch `CodexCLIProvider` from `codex -p <prompt>` to `codex exec --skip-git-repo-check --output-schema <FILE> --output-last-message <FILE>` | (a) keep `-p`, drop flag; (b) switch to `exec` + flag with `--output-format json`; (c) switch to `exec` + native schema flags | (c) — verified 2026-04-30 against codex 0.118.0 that `codex exec` does NOT accept `--output-format json`. The supported flags are `--output-schema <FILE>` (JSON Schema file describing response shape) and `--output-last-message <FILE>` (writes final agent message to file). Prompt is read from stdin via `-`. This brings Codex to **parity with Claude's `--json-schema`** and obsoletes the schema-in-prompt injection that the previous codex provider used. The provider writes the schema to a temp file, runs codex with both flags, then reads the parsed JSON from `--output-last-message`. `--skip-git-repo-check` is required because the worker runs from the data dir, not a git repo. |
...
| FR-B06 | `CodexCLIProvider` switches subprocess invocation from `codex -p <prompt> --model X` to `codex exec --skip-git-repo-check --model X --output-schema <SCHEMA_FILE> --output-last-message <OUT_FILE>` with the prompt piped on stdin via `-`. The provider writes the JSON schema to a temp file, invokes codex, then reads/parses the agent's final message from `<OUT_FILE>`. The pre-existing schema-in-prompt injection (`codex_cli.py:48-58`) is **deleted** — Codex now consumes the schema natively, matching Claude's `--json-schema` behavior. Both temp files are cleaned up after the call (success or failure). |
...
- `test_codex_cli_provider.py`:
  ...
  - subprocess argv includes `["codex", "exec", "--skip-git-repo-check", "--output-schema", <schema_path>, "--output-last-message", <out_path>, ...]`
  - schema is written to a temp file (not injected into prompt)
  - structured response is read from `--output-last-message` file and parsed as JSON
  - both temp files are cleaned up after the call (verify on both success and failure paths)
...
|---|----------|-------|-----------|
| OQ1 | ~~Confirm `codex exec` accepts the same `--model`, `--system-prompt`, stdin-prompt patterns we currently pass to `codex -p`. If not, the schema-in-prompt + stdin pattern may need to change shape.~~ **RESOLVED 2026-04-30:** codex 0.118 `exec` does NOT accept `--output-format json` (the proposed flag). It accepts `--output-schema <FILE>` + `--output-last-message <FILE>` natively, plus `--model`, `--skip-git-repo-check`, and `-` for stdin prompt. D3 + FR-B06 updated to use these native flags; schema-in-prompt injection is removed. | /plan code-study | resolved during /execute T3 smoke (codex 0.118.0) |
